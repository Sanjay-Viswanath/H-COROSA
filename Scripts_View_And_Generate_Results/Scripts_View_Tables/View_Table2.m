...
cd ..
fo = pwd;
fo1 = fullfile(fo,'Models_and_Measurements','Measurements_for_Table2');
fo2 = fullfile(fo,'Generated_Reconstruction_Results','Reconstructions_in_Table2');
fo3 = fullfile(fo,'Scripts_View_And_Generate_Results','Scripts_Table2_Entries');
...
for ind1 = 1:Ni
    ...
    for ind2 = 1:2
        ...
        if ind2 == 2
            fn3 = [fn1,'_Radial_SR_1_Sig_20'];
            load(fullfile(fo1,[fn3,'.mat']),'Im');
        else
            fn3 = [fn1,'_TSP_SR_1_Sig_20'];
            load(fullfile(fo1,[fn3,'.mat']),'Im');
        end
        ...
        for ind3 = 1:Nt
            ...
            Table_2(indr1,ind2) = round(real(ssim(s1*I1,Im,'Exponents',[1,1,1],'DynamicRange',255)),3);
            Table_2(indr1,ind2+2) = round(10*log10(sum(Im(:).^2)/sum((s2*I2(:) - Im(:)).^2)),2);
